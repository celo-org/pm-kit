...
**Verdicts:** `APPROVE` / `APPROVE-WITH-NITS` (= mergeable *now*; nits are not soft blockers) / `REQUEST-CHANGES` (only with a concrete blocking item). Nits that don't block get recorded at merge time as a follow-up issue (composer #452, #460) — otherwise they evaporate. **Review the tests as hard as the code:** if nothing realistic would make the suite go red, it's worse than none — it certifies broken output (composer #447 v1).
...
- **Required review-thread resolution (kit v2 ruleset) vs "approve-with-nits means mergeable now; nits are not soft blockers" (hardening).** Chose the playbook: dropped the thread-resolution rule. Mandatory resolution turns every nit into a click-blocker, which is exactly the soft-blocker pattern the playbook forbids. The mechanism that replaces it: every review point ends as commit / follow-up issue / explicit won't-do (§5).
